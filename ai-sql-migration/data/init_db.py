...
def _get_connection(
    ...
    - SQLEDGE_DATABASE (default: localdb)
    - SQLEDGE_USER
    - SQLEDGE_PASSWORD
    """
    ...
    database = database or os.environ.get("SQLEDGE_DATABASE", "localdb")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Initialize localdb on SQL Server or SQL Edge (schema + CSV load)."
    )
    parser.add_argument(
        "command",
        nargs="?",
...
def _connection_from_args(args: argparse.Namespace) -> tuple[str, int, str, str, str]:
    ...
    database = (
        ...
        else os.environ.get("SQLEDGE_DATABASE", "localdb")
    )
