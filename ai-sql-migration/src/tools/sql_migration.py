...
def _table_rewrite_pairs() -> list[tuple[str, str]]:
    """Longest-match-friendly (localuc_db.dbo.* before bare dbo.*)."""
    prefix = _migration_uc_prefix()
    pairs: list[tuple[str, str]] = []
    for t in _PHARMACY_DBO_TABLES:
        target = f"{prefix}.{t}"
        pairs.append((rf"\[localuc_db\]\.\[dbo\]\.\[{t}\]", target))
        pairs.append((rf"localuc_db\.dbo\.{t}\b", target))
...
def _rewrite_tsql_to_sparksql(query: str) -> tuple[str, list[str]]:
    ...
    # Remap localuc dbo.* → Unity Catalog gold (or SQL_MIGRATION_UC_PREFIX)
