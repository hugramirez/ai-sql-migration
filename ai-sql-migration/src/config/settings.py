...
When migrating, `migrate_sql_query` remaps `localuc_db.dbo.*` and `dbo.*` to `localuc.gold.*` by default (see `SQL_MIGRATION_UC_PREFIX`). Example:
| SQL Edge (T-SQL) | Databricks (Spark SQL) |
|---|---|
| `[localuc_db].[dbo].[dim_patient]` | `localuc.gold.dim_patient` |
...
class Settings:
    ...
    def from_env(cls) -> Settings:
        ...
        return cls(
            ...
            sqledge_database=os.environ.get("SQLEDGE_DATABASE", "localuc_db").strip(),
            ...
            sqlserver_database=os.environ.get("SQLSERVER_DATABASE", "localuc_db").strip(),
