...
def test_table_remap_dotted():
    sql, warnings = _rewrite_tsql_to_sparksql("SELECT id FROM localuc_db.dbo.dim_patient")
    assert "localuc.gold.dim_patient" in sql
    assert "localuc_db" not in sql
...
def test_table_remap_dbo_fact():
    ...
    assert "localuc.gold.fact_prescription" in sql
...
def test_uc_prefix_env_override(monkeypatch: pytest.MonkeyPatch):
    ...
    sql, _ = _rewrite_tsql_to_sparksql("SELECT 1 FROM localuc_db.dbo.dim_medication")
...
def test_migrate_sql_query_tool_output_format():
    query = "SELECT TOP 3 ISNULL(name, 'x') AS name FROM localuc_db.dbo.dim_patient;"
...
def test_migrate_sql_query_tool_migrated_sql_line():
    query = "SELECT TOP 3 GETDATE() AS ts FROM localuc_db.dbo.dim_patient;"
    result = migrate_sql_query.invoke({"query": query})
    ...
    assert "localuc.gold.dim_patient" in extracted
