...
def test_table_remap_dotted():
    sql, warnings = _rewrite_tsql_to_sparksql("SELECT id FROM localdb.dbo.dim_patient")
    assert "localuc.gold.dim_patient" in sql
    assert "localdb" not in sql
...
def test_uc_prefix_env_override(monkeypatch: pytest.MonkeyPatch):
    ...
    sql, _ = _rewrite_tsql_to_sparksql("SELECT 1 FROM localdb.dbo.dim_medication")
...
def test_migrate_sql_query_tool_output_format():
    query = "SELECT TOP 3 ISNULL(name, 'x') AS name FROM localdb.dbo.dim_patient;"
...
def test_migrate_sql_query_tool_migrated_sql_line():
    query = "SELECT TOP 3 GETDATE() AS ts FROM localdb.dbo.dim_patient;"
    result = migrate_sql_query.invoke({"query": query})
